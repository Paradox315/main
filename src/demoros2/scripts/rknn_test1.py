...
from demoros2.msg import Detection, DetectionsWithOdom
...
# 配置参数
ONNX_MODEL = '/home/ljy/demo2/src/demoros2/src/models/yolov8n.onnx'
RKNN_MODEL = '/home/ljy/demo2/src/demoros2/src/models/yolov8n.rknn'
QUANTIZE_ON = False
DATASET = '/home/ljy/demo2/src/demoros2/src/models/dataset.txt'
CLASSES_PATH = "/home/ljy/demo2/src/demoros2/src/models/coco.yaml"
...
class DataCollector:
    def __init__(self):
        ...
        rospy.loginfo("Data collector initialized with message_filters...")
    ...
    def sync_callback(self, pc_msg, img_msg, odom_msg):
        try:
            ...
            try:
                ...
                return
            self.data_queue.put_nowait((pc_msg, img_msg, odom_msg, self.latest_tf))
        except queue.Full:
            pass
...
class ThreadSafeRKNN:
    ...
    def run_detection(self, img, class_names):
        with self.lock:
            return RKNNProcessor.run_detection(self.rknn, img, class_names)
...
class FusionProcessor:
    def __init__(self):
        ...
        self.rknn = ThreadSafeRKNN()
        self.P_rect, self.R_rect, self.RT = LidarProcessor.load_calibration_data()
        self.detection_pub = rospy.Publisher(PUB_TOPIC, DetectionsWithOdom, queue_size=10)
    ...
    def worker_loop(self):
        while not self.stop_event.is_set():
            try:
                ...
                img, detections, pc_msg, odom_msg, transform = task
                ...
                rospy.logerr(f"Error in worker thread: {str(e)}")
    
    def process_data(self, pc_msg, img_msg, odom_msg, transform):
        """主处理流程"""
        try:
            img = self.bridge.compressed_imgmsg_to_cv2(img_msg, desired_encoding="bgr8")
            
            # 2. 运行目标检测
            detections, _ = self.rknn.run_detection(img, self.class_names)
            
            # 3. 将任务放入队列
            self.task_queue.put((img, detections, pc_msg, odom_msg, transform))
...
def main():
    try:
        ...
        while not rospy.is_shutdown():
            ...
            # 获取数据
            pc_msg, img_msg, odom_msg, tf_msg = collector.get_data()
            if pc_msg is None or img_msg is None or odom_msg is None or tf_msg is None:
                rospy.sleep(0.01)
                continue
                
            print("Processing new frame...")
            start_time = time.time()
            success = processor.process_data(pc_msg, img_msg, odom_msg, tf_msg)
