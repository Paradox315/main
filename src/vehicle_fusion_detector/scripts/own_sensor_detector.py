...
class OwnSensorDetector:
    def __init__(self):
        ...
        rospy.loginfo(f"Own Sensor Detector initialized for {self.vehicle_id}")
        ...
        try:
            with rosbag.Bag(self.bag_file, "r") as bag:
                for topic, msg, t in bag.read_messages(topics=[self.input_topic]):
                    ...
                    # 如果是其他格式，需要转换（这里假设是Detections格式）
    ...
    def create_default_data(self):
        ...
        for i in range(10):  # 创建10条测试数据
            ...
            for j in range(num_detections):
                ...
                detection.object_id = f"own_obj_{j}"
                detection.type = "car"
                detection.confidence = random.uniform(0.6, 0.95)
                detection.box_2d = [
                    int(random.uniform(1.5, 9.5)),
                    int(random.uniform(1.5, 9.5)),
                    int(random.uniform(11, 18)),
                    int(random.uniform(11, 18)),
                ]
    ...
    def publish_next_message(self):
        ...
        rospy.loginfo(
            f"Vehicle {self.vehicle_id} (own) publishing {len(msg.detections)} detections."
        )
...
if __name__ == "__main__":
    try:
        detector = OwnSensorDetector()
        detector.run()
    except rospy.ROSInterruptException:
        rospy.loginfo("Own Sensor Detector shutdown.")
